...
#from util.segmentation import remove_silence
...
def predict_anxiety_level(data_path, text, print_prediction, model_name="fusion_balanced.h5", weights_name="fusion_weights.h5"):
...
def predict_anxiety_level_audio(audio_path, text, print_prediction, model_name="cnn.h5",
                          weights_name="cnn.h5"):
    #remove_silence(audio_path, audio_path)

    model_path = audio_path + model_name
    weights_path = audio_path + weights_name
    json_file = open(model_path, 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    ...
    with open(audio_path + 'tokenizer.pickle', 'rb') as handle:
        tokenizer = pickle.load(handle)
